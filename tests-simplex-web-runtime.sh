...
assert_file_contains "$ROOT_DIR/src/browser-simplex-contact-client.mjs" 'initialConfirmation' 'browser SimpleX contact client persists encrypted initial contact retry packets'
...
assert_file_contains "$ROOT_DIR/src/browser-simplex-contact-client.mjs" 'sendReadReceipt' 'browser SimpleX contact client sends encrypted read receipts'
...
assert_file_contains "$ROOT_DIR/tests/browser-simplex-contact-client.test.mjs" 'outbound queue is missing' 'browser SimpleX contact client tests missing outbound queue ratchet rollback'
assert_file_contains "$ROOT_DIR/tests/browser-simplex-contact-client.test.mjs" 'failed contact requests without storing plaintext profile data' 'browser SimpleX contact client tests plaintext-free initial contact retries'
assert_file_contains "$ROOT_DIR/tests/browser-simplex-contact-client.test.mjs" 'failed accept confirmations after saving active contact state' 'browser SimpleX contact client tests plaintext-free accept confirmation retries'
assert_file_contains "$ROOT_DIR/tests/browser-simplex-contact-client.test.mjs" 'keeps accept confirmation state and retries ACK after ack transport failure' 'browser SimpleX contact client tests accept-confirmation ACK retry'
assert_file_contains "$ROOT_DIR/tests/browser-simplex-contact-client.test.mjs" 'saves request state sends accept and retries ACK after request ack failure' 'browser SimpleX contact client tests contact-request ACK retry'
assert_file_contains "$ROOT_DIR/tests/browser-simplex-contact-client.test.mjs" 'pending.payload.packet instanceof Uint8Array' 'browser SimpleX contact client tests binary retry packet storage'
...
assert_file_contains "$ROOT_DIR/tests/browser-simplex-contact-client.test.mjs" 'sends and receives encrypted read receipts' 'browser SimpleX contact client tests encrypted read receipts'
assert_file_contains "$ROOT_DIR/src/browser-simplex-client.mjs" 'sendPreparedTransmission' 'browser SimpleX client can resend prepared encrypted transmissions'
...
assert_file_contains "$ROOT_DIR/src/browser-simplex-web-transport-adapter.mjs" 'sendReadReceipt' 'browser SimpleX web adapter exposes read receipts'
...
assert_file_contains "$ROOT_DIR/tests/browser-simplex-web-transport-adapter.test.mjs" "transport_status, 'read'" 'browser SimpleX web adapter has E2E read receipt coverage'
